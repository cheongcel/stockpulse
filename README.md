...
초기 버전은 "AI 감성 분석 기반 투자 판단 서비스"를 표방했지만, 실제로는 구현되지 않은 기능이
화면에 표시되고 핵심 구독 기능에도 결함이 있었습니다. 서비스 범위를
**"키워드 검색 + 구독 + 뉴스 요약 다이제스트"** 로 좁히고 약속한 기능이 제대로 동작하도록 정리했습니다.

**범위 축소 — 제거한 것**
- 감성 분석 / 긍정·부정 점수 / 투자 판단 문구
- DART 연동, Slack 발송, ETF 리포트, 포트폴리오 성과 등 미구현 홍보 문구
- 사용되지 않던 `NewsAnalysis` 엔티티

**동작 개선 — 고친 것**

| 기존 문제 | 개선 |
|---|---|
| 제목만으로 여러 기사를 하나의 시장 평가로 통합 | 제목+본문 일부 기반 **기사별 한 줄 요약** + 원문 링크, 근거 없는 추측 금지 |
| 정확도순 수집이라 오래된 기사가 섞임 | **최신순 수집**, 중복 링크 제거 |
| 키워드가 DB 고유값 → 한 키워드를 한 사람만 구독 가능 | 고유 기준을 **(키워드, 이메일) 조합**으로 변경 |
| 공개 화면의 구독 목록·삭제 버튼 → 누구나 남의 구독 해지 가능 | 목록 제거, **구독자별 토큰**으로 이메일 속 전용 해지 링크만 허용 |
| 이메일 소유 확인 없이 즉시 구독 | **더블 옵트인** — 확인 메일의 링크를 눌러야 발송 시작 + 서버 측 입력 검증 |
| AI 실패 문구가 정상 요약처럼 발송됨 | 뉴스 없음 / 수집 실패 / 요약 실패를 구분, 요약만 실패 시 제목·링크는 정상 제공 |
| 뉴스 목록과 요약을 따로 캐싱 → 화면과 메일 내용 불일치 | `DigestService`로 **한 묶음 캐싱**해 항상 동일한 결과 보장 |
| 발송 성공 여부 미기록, 중복 발송 가능 | 발송 이력 기록, **새 기사 없으면 생략**, 실패 건만 다음 회차 재시도 |
| 스케줄러 타임존 미지정, 메일 링크가 이전 주소 | `Asia/Seoul` 명시, 링크를 현재 도메인(`APP_BASE_URL`)으로 통일 |
| viewport 미설정으로 모바일에서 화면 축소 | viewport 추가 + 반응형 레이아웃 적용 |
...
> 구독 흐름, 실패 처리, 데이터 모델 등 상세 설명은 **[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)** 를 참고하세요.
...
- [ ] **Test Coverage:** 동일 키워드 다중 구독, 본인 구독 해지, 외부 API 실패, 중복 발송 방지에 대한
      JUnit5/Mockito 테스트
- [x] **Mobile-friendly UI:** viewport 설정 및 반응형 레이아웃 적용
- [x] **키워드 미리보기:** 구독 없이 키워드만 입력해 최신 뉴스·요약을 먼저 확인 가능
